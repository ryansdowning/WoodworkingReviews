...
export interface WWReviewsSidebarMenuItemProps extends AbstractSideBarItem {
  icon: ReactNode;
  link?: string;
  sublinks?: SidebarItemLink[];
}

export function WWReviewsSidebarMenuItem({
  title,
  icon,
  sublinks,
  link,
...
export function WWReviewsSidebar() {
  const router = useRouter();
  const dark = useDark();
      ...
      >
        <Stack sx={{ width: EXPANDED_SIDEBAR_WIDTH - SIDEBAR_X_PADDING * 2 - 1 }}>
          ...
          <Group align="center" spacing="xs">
            <ColorSchemeToggle />
            <Text size="sm">{dark ? "Light" : "Dark"} Mode</Text>
          </Group>
        </Stack>
      </Navbar>
